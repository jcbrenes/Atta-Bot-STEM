...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{3.6cm} X @{}}
        ...
        Función que la pila BLE ejecuta automáticamente al conectarse o desconectarse con un dispositivo; nunca se llama manualmente. En el código: clase \texttt{BLEServerCallbacks}, métodos \texttt{onConnect()} / \texttt{onDisconnect()}. \\
        \addlinespace
        Servicio &
        ...
        Determinan qué operaciones puede realizar el dispositivo remoto sobre ese dato: lectura, escritura, o ambas. En el código: \texttt{PROPERTY\_READ}, \texttt{PROPERTY\_WRITE}. \\
        \addlinespace
...
Dado que \texttt{BLEDevice} funciona mediante eventos asíncronos no es posible ``preguntar'' en cada iteración de \texttt{loop()} si llegó un mensaje nuevo, por lo que el proyecto define dos clases de callback. La de escritura sobre el característico es la más relevante para el protocolo de mensajes:
...
\textbf{Nota 1.} En todos los comandos con \texttt{\#\#\#}, éste representa un número entero de 3 dígitos sin signo.
